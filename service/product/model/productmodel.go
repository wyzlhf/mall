...
type (
	...
	ProductModel interface {
		productModel
		TxAdjustStock(ctx context.Context, tx *sql.Tx, id uint64, delta int) (sql.Result, error)
	}
...
func (m *defaultProductModel) TxAdjustStock(ctx context.Context, tx *sql.Tx, id uint64, delta int) (sql.Result, error) {
	productIdKey := fmt.Sprintf("%s%v", cacheProductIdPrefix, id)
	return m.Exec(func(conn sqlx.SqlConn) (sql.Result, error) {
		query := fmt.Sprintf("update %s set stock=stock+? where stocke>=-? and id=?", m.table)
